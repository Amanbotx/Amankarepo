...
async def start(client, message):
    if message.chat.type in [enums.ChatType.GROUP, enums.ChatType.SUPERGROUP]:
        buttons = [[
                    InlineKeyboardButton('☆ ᴀᴅᴅ ᴍᴇ ᴛᴏ ʏᴏᴜʀ ɢʀᴏᴜᴘ ☆', url=f'http://t.me/movie_090_bot?startgroup=true')
                ],[
                    InlineKeyboardButton('🍁 ʜᴏᴡ ᴛᴏ ᴜꜱᴇ 🍁', url="https://t.me/movie_090_bot?start=help")
                  ]]
    ...
    if len(message.command) != 2:
        ...
        await message.reply_photo(
            ...
            caption=script.START_TXT.format(message.from_user.mention, gtxt, message.from_user.id, temp.U_NAME, temp.B_NAME),
        ...
        for file in files:
            ...
            if not await check_verification(client, message.from_user.id) and VERIFY == True:
                btn = [[
                    ...
                    InlineKeyboardButton("⁉️ ʜᴏᴡ ᴛᴏ ᴠᴇʀɪꜰʏ ⁉️", url=await get_tutorial(chat_id))
                ]]
                await message.reply_text(
